...
model=streamspeech.offline-s2st.${LANG}-en.new
...
fairseq-train $DATA \
  ...
  --encoder-layers 6 --encoder-embed-dim 256 --encoder-ffn-embed-dim 2048 --encoder-attention-heads 2 \
  --translation-decoder-layers 4 --synthesizer-encoder-layers 2 \
  --decoder-layers 2 --decoder-embed-dim 512 --decoder-ffn-embed-dim 2048 --decoder-attention-heads 2 \
  --k1 0 --k2 0 --n1 1 --n2 -1 \
  --chunk-size 8000 \
  ...
  --save-interval 5 --save-interval-updates 1000 \
  --keep-last-epochs -1 \
  --max-epoch $EPOCHS \
